...
`GpxReader` also exposes `getWaypointIterator()` for named waypoints and `getRoutePositionIterator()` for planned routes. For NMEA 0183 sentence parsing, use `gov.nasa.worldwind.formats.nmea.NmeaReader`.
...
VPFDatabase db = VPFUtils.readDatabase("data/vmaplv0/");
